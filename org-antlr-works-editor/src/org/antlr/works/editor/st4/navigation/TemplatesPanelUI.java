...
import org.antlr.works.editor.st4.navigation.actions.FilterSubmenuAction;
...
public class TemplatesPanelUI extends javax.swing.JPanel implements ExplorerManager.Provider, FiltersManager.FilterChangeListener, PropertyChangeListener {
    private static final Rectangle ZERO = new Rectangle(0, 0, 1, 1);
    private static final Logger PERF_LOG = Logger.getLogger(TemplatesPanelUI.class.getName() + ".perf");
    private static final RequestProcessor RP = new RequestProcessor(TemplatesPanelUI.class.getName(), 1);
    ...
    private final TemplatesFilters filters;
    ...
    public TemplatesPanelUI() {
        ...
        filtersPanel.setToolTipText(NbBundle.getMessage(TemplatesPanelUI.class, "TIP_CollapsiblePanel", keyText));

        filters = new TemplatesFilters(this);
        ...
        explorerManager.setRootContext(TemplateNode.getWaitNode());

        boolean expanded = NbPreferences.forModule(TemplatesPanelUI.class).getBoolean("filtersCollapsiblePanel.expanded", true);
...
//    }
    ...
    public TemplatesFilters getFilters() {
        return filters;
    }
    ...
    private TemplateNode getRootNode() {
    ...
    public void propertyChange(PropertyChangeEvent event) {
        if (ExplorerManager.PROP_SELECTED_NODES.equals(event.getPropertyName())) {
            ...
            NbPreferences.forModule(TemplatesPanelUI.class).putBoolean("filtersCollapsiblePanel.expanded", filtersPanel.isExpanded());
        }
    }

    public void refresh(final Description description) {
        final TemplateNode rootNode = getRootNode();
        if (rootNode != null && rootNode.getDescription().fileObject.equals(description.fileObject)) {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                    ...
                    public void run() {
                        ...
                        explorerManager.setRootContext(new TemplateNode(description));
