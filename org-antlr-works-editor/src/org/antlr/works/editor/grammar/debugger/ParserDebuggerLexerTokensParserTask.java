 ...
 * @author Sam Harwell
 */
public final class ParserDebuggerLexerTokensParserTask implements ParserTask {

    private static final String DOCUMENT_CACHE_KEY = ParserDebuggerLexerTokensParserTask.class.getName() + "-snapshot-data";

    private final Object lock = new Object();

    private ParserDebuggerLexerTokensParserTask() {
    }
    ...
    public void parse(ParserTaskManager taskManager, ParseContext context, DocumentSnapshot snapshot, Collection<? extends ParserDataDefinition<?>> requestedData, ParserResultHandler results)
        ...
        if (requestedData.contains(ParserDebuggerParserDataDefinitions.LEXER_TOKENS)) {
            ...
            synchronized (documentCache) {
                ...
                if (result == null) {
                    ...
                    if (previousResult != null) {
                        ParserDebuggerTokensTaskTaggerSnapshot previousTagger = (ParserDebuggerTokensTaskTaggerSnapshot)previousResult.getData();
                        result = new BaseParserData<>(context, ParserDebuggerParserDataDefinitions.LEXER_TOKENS, snapshot, previousTagger.translateTo(snapshot));
                    } else {
                        ParserDebuggerTokensTaskTaggerSnapshot tagger = new ParserDebuggerTokensTaskTaggerSnapshot(snapshot);
                        tagger.initialize();
                        result = new BaseParserData<>(context, ParserDebuggerParserDataDefinitions.LEXER_TOKENS, snapshot, tagger);
                    }
    ...
    private static final class Definition extends ParserTaskDefinition {
        ...
        private static final Collection<ParserDataDefinition<?>> OUTPUTS =
            Collections.<ParserDataDefinition<?>>singletonList(ParserDebuggerParserDataDefinitions.LEXER_TOKENS);

        public static final Definition INSTANCE = new Definition();

        public Definition() {
            super("Parser Debugger Lexer Tokens", INPUTS, OUTPUTS, ParserTaskScheduler.CONTENT_SENSITIVE_TASK_SCHEDULER);
        }
    }
    ...
    public static final class Provider extends DocumentParserTaskProvider {
        ...
        public ParserTask createTaskImpl(VersionedDocument document) {
            return new ParserDebuggerLexerTokensParserTask();
        }
